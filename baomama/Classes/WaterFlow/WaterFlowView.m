...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (NSIndexPath * indexPath in keyArray) {
        ...
        if (CGRectContainsPoint(cell.frame, location)) {
            CGRect left = cell.frame;
            left.size.width *= 0.5;
            if (CGRectContainsPoint(left, location)) {
                cell.isClickLeft = YES;
            }
            else
            {
                cell.isClickLeft = NO;
            }
            if ([self.delegate respondsToSelector:@selector(waterFlowView:didSelectRowAtIndexPath:waterFlowViewCell:)]) {
                [self.delegate waterFlowView:self didSelectRowAtIndexPath:indexPath waterFlowViewCell:cell];
                break;
            }
        }
    }
...
//    BBLog(@"layout subviews %d %f %f", self.subviews.count, self.contentOffset.y,self.contentSize.height);
